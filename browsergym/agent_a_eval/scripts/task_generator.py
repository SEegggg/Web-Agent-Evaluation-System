...
然后独立的评估 Agent 根据你定义的评估标准对 Agent A 的表现进行评分。"""
    ...
    def validate(self, md_content: str) -> list[str]:
        ...
        # 5. 评估标准 should have at least one ### sub-section (benchmark format)
        eval_match = re.search(r"## 评估标准\n+(.*?)(?=\n## |\Z)", md_content, re.DOTALL)
        if eval_match:
            eval_body = eval_match.group(1)
            if not re.search(r"^###\s+", eval_body, re.MULTILINE):
                errors.append("'## 评估标准' 中缺少 '### ' 子章节（需使用 benchmark 格式）")
...
def load_config(config_path: str) -> dict:
    ...
    if api_key is None:
        import os
        if llm_section.get("provider", "openai") == "openai":
            api_key = os.environ.get("OPENAI_API_KEY")
        elif llm_section.get("provider") == "anthropic":
